...
RR 5==>   -----   ------  <== RL 4
         |-----   ------|
*/
#define EEPROM_TRIM false  //暂时不使用
...
#define PIN_RR 5 //对应舵机在主板上的管脚5
#define PIN_RL 4 //对应舵机在主板上的管脚4
#define PIN_YR 3 //对应舵机在主板上的管脚3
#define PIN_YL 2 //对应舵机在主板上的管脚2

#define INTERVALTIME 10.0 

Oscillator servo[N_SERVOS];

//这一列函数都是跳舞的某个动作，看函数名猜意思，看不懂的找百度翻译
...
void setup()
{
  ...
  servo[0].attach(PIN_RR); //舵机初始化，绑定对应管脚
  servo[1].attach(PIN_RL); //舵机初始化，绑定对应管脚
  servo[2].attach(PIN_YR); //舵机初始化，绑定对应管脚
  servo[3].attach(PIN_YL); //舵机初始化，绑定对应管脚
...
// TEMPO: 121 BPM
int t=495; //t= 495ms 后面的dance函数里虎用到
double pause=0;

void loop()
{
     dance(); //跳舞，
     for(int i=0;i<4;i++) servo[i].SetPosition(90); //调完舞后 所有舵机回中 90度 立正状态
}

void dance(){
  //这里一系列的函数都是跳舞动作的某个分解动作的一个部分，串起来就是一个舞蹈
  primera_parte();
  segunda_parte();
  moonWalkLeft(4,t*2); //4是走的步数 t*2是在t*2的时间里做完这个动作 
  moonWalkRight(4,t*2);
  moonWalkLeft(4,t*2);
  moonWalkRight(4,t*2);
  primera_parte(); 
